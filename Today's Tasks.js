...
// ?listName=[[listname]]&listPosition=&tasks=[[tasklist]]&listPosition=bottom
    ...
    const url = encodeURI(`clearapp://list/create?listName=Today&tasks=${generateClearList()}&listPosition=bottom`);
    console.log(url)
...
function getNotes(item) {
  return item.children.filter(({type}) => type === 'note').map(({value}) => value).join('');
}
...
function generateRow(item, i) {
    ...
    const titleCell = row.addText(generateRowTitle(item), generateRowSubtitle(item));
