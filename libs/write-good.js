...
var re$1 = new RegExp('\\b(am|are|were|being|is|been|was|be)\\b\\s*([\\w]+ed|' + irregulars.join('|') + ')\\b', 'gi');
var byRe; // lazly construct

var passive = function (text, options) {
  var r = (options && options.by) ?
          (byRe || constructByRe()) : re$1; // not sorry

  var suggestions = [];
  var match;
  while (match = r.exec(text)) {
    if (exceptions$1.indexOf(match[2].toLowerCase()) === -1) {
      suggestions.push({
        index: match.index,
...
function constructByRe () {
  return byRe = new RegExp(re$1.toString().slice(1, -3) + '\\s*by\\b', 'gi');
}
...
var adverbs_1 = function matchAdverbs(text) {
  return matcher_1(adverbRegex, text);
};

function matcher$1(regex, text) {
  const results = [];
  let result = regex.exec(text);
...
var tooWordy = function isTextWordy(text) {
  return matcher_1$1(wordyRegex, text);
};

function matcher$2(regex, text) {
  const results = [];
  let result = regex.exec(text);
...
var cliches_1 = function clichesMatcher(text) {
  return matcher_1$2(clicheRegex, text);
};
...
var re$2 = new RegExp('\\b(' + toBe.join('|') + ')\\b', 'gi');

var ePrime = function (text) {
    var suggestions = [];
    if (!text || text.length === 0) return suggestions;
    text = text.replace(/[\u2018\u2019]/g, "'"); // convert smart quotes
    while (match = re$2.exec(text)) {
...
const re$3 = new RegExp('(\\s*)([^\\s]+)', 'gi');
const word = /\w+/;

var lexicalIllusions = function lexicalIllusions(text) {
  const suggestions = [];
  let lastMatch = '';
  let match;

  // eslint-disable-next-line no-cond-assign
  while (match = re$3.exec(text)) {
...
const re$4 = new RegExp('([^\n\\.;!?]+)([\\.;!?]+)', 'gi');
const startsWithSoRegex = new RegExp('^(\\s)*so\\b[\\s\\S]', 'i');

var startsWithSo = function startsWithSo(text) {
  const suggestions = [];
  let match;
  let innerMatch;

  while (match = re$4.exec(text)) {
...
var thereIs = function startsWithThereIs(text) {
  const suggestions = [];
  let match;
  let innerMatch;

  while (match = re$5.exec(text)) {
...
var annotate = function annotate(contents, suggestions, parse) {
...
const defaultChecks = {
  weasel: { fn: weasel, explanation: 'is a weasel word' },
  ...
  passive: { fn: passive, explanation: 'may be passive voice' },
  adverb: { fn: adverbs_1, explanation: 'can weaken meaning' },
  tooWordy: { fn: tooWordy, explanation: 'is wordy or unneeded' },
  cliches: { fn: cliches_1, explanation: 'is a cliche' },
...
var writeGood = function writeGood(text, opts = {}) {
  const finalOpts = {};
...
var annotate$1 = annotate;
writeGood.annotate = annotate$1;

module.exports.annotate = annotate$1;
module.exports.default = writeGood;
