...
const P = importModule('parsimmon');
...
const PROJECT = P.seq(P.index, P.regex(/([^\n]+?):/, 1), TAGS)
  .skip(NEWLINE)
...
const TASK = P.seq(P.index, P.string('- '), NON_TAG_STRING, TAGS)
  .skip(NEWLINE)
...
const NOTE = P.seq(P.index, P.regex(/[^-\n]([^\n]*[^:\n])?\n*/))
...
function leafBlock(depth = 1, prefix = '') {
  return (
    P.seq(P.string(prefix), depth === 1 ? P.string('') : INDENT, NOTE)
...
function parentBlock(depth = 1, prefix = '') {
  return P.seq(
    ...
    return block(depth + 1, prefix + indent)
      .many()
      .map(children => {
        let out = item;
        out.depth = depth;
...
function parse(str) {
  ...
  if (out.status) {
    ...
    let err = new Error(
      `Parse error in line ${out.index.line}, expected ${out.expected.join(
        ' or '
      )}`
    );
    err.index = out.index;
